...
app.get('/', (req, res) => {
  res.status(200).json({
    ...
    endpoints: {
      products: `${req.protocol}://${req.get('host')}${config.apiPrefix}/products`,
      sales: `${req.protocol}://${req.get('host')}${config.apiPrefix}/sales`,
      dashboard: `${req.protocol}://${req.get('host')}${config.apiPrefix}/dashboard`
    }
  });
});
...
${config.isDevelopment ? '🔧 Development Mode - Detailed logging enabled' : '🔒 Production Mode - Security enhanced'}
  `);
});
